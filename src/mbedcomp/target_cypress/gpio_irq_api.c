...
void gpio_irq_set(gpio_irq_t *obj, gpio_irq_event event, uint32_t enable) {
    ...
    if (!enable)
        ...
        CY_SET_REG8(obj->inttypeReg, CY_GET_REG8(obj->inttypeReg) | 0x01);
    else if (event == IRQ_FALL)
        CY_SET_REG8(obj->inttypeReg, CY_GET_REG8(obj->inttypeReg) | 0x02);
    else if (event == (IRQ_FALL | IRQ_RISE))
        CY_SET_REG8(obj->inttypeReg, CY_GET_REG8(obj->inttypeReg) | 0x03);
    else
