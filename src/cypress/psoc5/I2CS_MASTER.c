...
volatile uint8 I2CS_mstrStatus;     /* Master Status byte  */
volatile uint8 I2CS_mstrControl;    /* Master Control byte */

/* Transmit buffer variables */
volatile uint8 * I2CS_mstrRdBufPtr;     /* Pointer to Master Read buffer */
volatile uint8   I2CS_mstrRdBufSize;    /* Master Read buffer size       */
volatile uint8   I2CS_mstrRdBufIndex;   /* Master Read buffer Index      */

/* Receive buffer variables */
volatile uint8 * I2CS_mstrWrBufPtr;     /* Pointer to Master Write buffer */
volatile uint8   I2CS_mstrWrBufSize;    /* Master Write buffer size       */
volatile uint8   I2CS_mstrWrBufIndex;   /* Master Write buffer Index      */


/*******************************************************************************
* Function Name: I2CS_MasterWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CS_mstrStatus  - The global variable used to store a current
*                                 status of the I2C Master.
*  I2CS_state       - The global variable used to store a current
*                                 state of the software FSM.
*  I2CS_mstrControl - The global variable used to control the master
*                                 end of a transaction with or without Stop
*                                 generation.
*  I2CS_mstrWrBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  I2CS_mstrWrBufIndex - The global variable used to store current
*                                    index within the master write buffer.
*  I2CS_mstrWrBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No
      ...
      
{
    ...
    errStatus = I2CS_MSTR_NOT_READY;

    if(NULL != wrData)
    {
        /* Check I2C state to allow transfer: valid states are IDLE or HALT */
        if(I2CS_SM_IDLE == I2CS_state)
        {
            /* Master is ready for transaction: check if bus is free */
            if(I2CS_CHECK_BUS_FREE(I2CS_MCSR_REG))
            {
                errStatus = I2CS_MSTR_NO_ERROR;
            }
            else
            {
                errStatus = I2CS_MSTR_BUS_BUSY;
            }
        }
        else if(I2CS_SM_MSTR_HALT == I2CS_state)
        {
            /* Master is ready and waiting for ReStart */
            errStatus = I2CS_MSTR_NO_ERROR;

            I2CS_ClearPendingInt();
            I2CS_mstrStatus &= (uint8) ~I2CS_MSTAT_XFER_HALT;
        }
        else
        {
            /* errStatus = I2CS_MSTR_NOT_READY was send before */
        }

        if(I2CS_MSTR_NO_ERROR == errStatus)
        {
            /* Set state to start write transaction */
            I2CS_state = I2CS_SM_MSTR_WR_ADDR;

            /* Prepare write buffer */
            I2CS_mstrWrBufIndex = 0u;
            I2CS_mstrWrBufSize  = cnt;
            I2CS_mstrWrBufPtr   = (volatile uint8 *) wrData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            I2CS_mstrControl = mode;

            /* Clear write status history */
            I2CS_mstrStatus &= (uint8) ~I2CS_MSTAT_WR_CMPLT;

            /* Hardware actions: write address and generate Start or ReStart */
            I2CS_DATA_REG = (uint8) (slaveAddress << I2CS_SLAVE_ADDR_SHIFT);

            if(I2CS_CHECK_RESTART(mode))
            {
...
/*******************************************************************************
* Function Name: I2CS_MasterReadBuf
********************************************************************************
*
* Summary:
*  Automatically writes an entire buffer of data to a slave device. Once the
...
*                                 Stop generation.
*  I2CS_mstrRdBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  I2CS_mstrRdBufIndex - The global variable  used to store a
*                                    current index within the master
*                                    write buffer.
*  I2CS_mstrRdBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(NULL != rdData)
    {
        ...
        {
            /* errStatus = I2CS_MSTR_NOT_READY was set before */
        }

        if(I2CS_MSTR_NO_ERROR == errStatus)
        {
            /* Set state to start write transaction */
            I2CS_state = I2CS_SM_MSTR_RD_ADDR;

            /* Prepare read buffer */
            I2CS_mstrRdBufIndex  = 0u;
            I2CS_mstrRdBufSize   = cnt;
            I2CS_mstrRdBufPtr    = (volatile uint8 *) rdData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            I2CS_mstrControl = mode;

            /* Clear read status history */
            I2CS_mstrStatus &= (uint8) ~I2CS_MSTAT_RD_CMPLT;
...
*
* Global variables:
*  I2CS_state - The global variable used to store a current state of
*                           the software FSM.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(I2CS_SM_IDLE == I2CS_state)
    {
        ...
        if(I2CS_CHECK_BUS_FREE(I2CS_MCSR_REG))
        {
            ...
            slaveAddress = (uint8) (slaveAddress << I2CS_SLAVE_ADDR_SHIFT);
            if(0u != R_nW)
            {
                slaveAddress |= I2CS_READ_FLAG;
                I2CS_state = I2CS_SM_MSTR_RD_ADDR;
            }
            else
            {
            ...
            I2CS_DATA_REG = slaveAddress;
            I2CS_GENERATE_START_MANUAL;

            /* Wait until address is transferred */
            while(I2CS_WAIT_BYTE_COMPLETE(I2CS_CSR_REG))
            {
            }
            ...
            if(I2CS_CHECK_START_GEN(I2CS_MCSR_REG))
            {
                ...
                errStatus = I2CS_MSTR_ERR_ABORT_START_GEN;
            }
            else
        #endif /* (I2CS_MODE_MULTI_MASTER_SLAVE_ENABLED) */

        #if(I2CS_MODE_MULTI_MASTER_ENABLED)
            if(I2CS_CHECK_LOST_ARB(I2CS_CSR_REG))
            {
                I2CS_BUS_RELEASE_MANUAL;

                /* Master lost arbitrage: reset FSM to IDLE */
                I2CS_state = I2CS_SM_IDLE;
                errStatus = I2CS_MSTR_ERR_ARB_LOST;
            }
            else
        #endif /* (I2CS_MODE_MULTI_MASTER_ENABLED) */

            if(I2CS_CHECK_ADDR_NAK(I2CS_CSR_REG))
            {
                /* Address has been NACKed: reset FSM to IDLE */
                I2CS_state = I2CS_SM_IDLE;
                errStatus = I2CS_MSTR_ERR_LB_NAK;
            }
            else
            {
      ...
      
{
    ...
    if(I2CS_CHECK_MASTER_MODE(I2CS_MCSR_REG))
    {
        /* Set address and read/write flag */
        slaveAddress = (uint8) (slaveAddress << I2CS_SLAVE_ADDR_SHIFT);
        if(0u != R_nW)
...
uint8 I2CS_MasterSendStop(void) 
{
    ...
    if(I2CS_CHECK_MASTER_MODE(I2CS_MCSR_REG))
    {
        I2CS_GENERATE_STOP_MANUAL;
        I2CS_state = I2CS_SM_IDLE;

        /* Wait until stop has been generated */
        while(I2CS_WAIT_STOP_COMPLETE(I2CS_CSR_REG))
        {
        }
...
uint8 I2CS_MasterWriteByte(uint8 theByte) 
{
    ...
    if(I2CS_CHECK_MASTER_MODE(I2CS_MCSR_REG))
    {
        I2CS_DATA_REG = theByte;   /* Write DATA register */
        I2CS_TRANSMIT_DATA_MANUAL; /* Set transmit mode   */
        ...
        if(I2CS_CHECK_DATA_ACK(I2CS_CSR_REG))
        {
            I2CS_state = I2CS_SM_MSTR_HALT;
            errStatus = I2CS_MSTR_NO_ERROR;
        }
        else
        {
...
uint8 I2CS_MasterReadByte(uint8 acknNak) 
{
    ...
    if(I2CS_CHECK_MASTER_MODE(I2CS_MCSR_REG))
    {
        ...
        */
        if(I2CS_SM_MSTR_RD_ADDR == I2CS_state)
        {
            I2CS_READY_TO_READ_MANUAL;
...
/*******************************************************************************
* Function Name: I2CS_MasterStatus
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2CS_MasterClearStatus
********************************************************************************
*
* Summary:
...
uint8 I2CS_MasterClearStatus(void) 
{
    ...
    status = I2CS_mstrStatus;
    I2CS_mstrStatus = I2CS_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: I2CS_MasterGetReadBufSize
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CS_mstrRdBufIndex - The global variable stores current index
*                                    within the master read buffer.
*
*******************************************************************************/
uint8 I2CS_MasterGetReadBufSize(void) 
{
    return (I2CS_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: I2CS_MasterGetWriteBufSize
********************************************************************************
*
* Summary:
*  Returns the amount of bytes that has been transferred with an
...
*
* Global variables:
*  I2CS_mstrWrBufIndex -  The global variable used to stores current
*                                     index within master write buffer.
*
*******************************************************************************/
uint8 I2CS_MasterGetWriteBufSize(void) 
{
    return (I2CS_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: I2CS_MasterClearReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CS_mstrRdBufIndex - The global variable used to stores current
...
/*******************************************************************************
* Function Name: I2CS_MasterClearWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CS_mstrRdBufIndex - The global variable used to stote current
*                                    index within master read buffer.
*  I2CS_mstrStatus - The global variable used to store a current
*                                status of the I2C Master.
*
