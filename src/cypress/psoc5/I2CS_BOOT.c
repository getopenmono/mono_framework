...
static uint8 XDATA I2CS_slReadBuf[I2CS_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA I2CS_slWriteBuf[I2CS_BTLDR_SIZEOF_WRITE_BUFFER];
...
*
* Global variables:
*  I2CS_slWriteBuf - The global variable used to store a received
*                                command.
*  I2CS_slReadBuf -  The global variable used to store a response.
...
void I2CS_CyBtldrCommStart(void) CYSMALL 
{
    ...
    I2CS_SlaveInitWriteBuf(I2CS_slWriteBuf, I2CS_BTLDR_SIZEOF_WRITE_BUFFER);
    I2CS_SlaveInitReadBuf (I2CS_slReadBuf, 0u);
...
*
*******************************************************************************/
cystatus I2CS_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
*
* Global variables:
*  I2CS_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (I2CS_slStatus & I2CS_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = I2CS_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2CS_slWriteBuf,
                              I2CS_MIN_UNT16(byteCount, size));
