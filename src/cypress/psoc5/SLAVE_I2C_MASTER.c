...
volatile uint8 SLAVE_I2C_mstrStatus;     /* Master Status byte  */
volatile uint8 SLAVE_I2C_mstrControl;    /* Master Control byte */

/* Transmit buffer variables */
volatile uint8 * SLAVE_I2C_mstrRdBufPtr;     /* Pointer to Master Read buffer */
volatile uint8   SLAVE_I2C_mstrRdBufSize;    /* Master Read buffer size       */
volatile uint8   SLAVE_I2C_mstrRdBufIndex;   /* Master Read buffer Index      */

/* Receive buffer variables */
volatile uint8 * SLAVE_I2C_mstrWrBufPtr;     /* Pointer to Master Write buffer */
volatile uint8   SLAVE_I2C_mstrWrBufSize;    /* Master Write buffer size       */
volatile uint8   SLAVE_I2C_mstrWrBufIndex;   /* Master Write buffer Index      */


/*******************************************************************************
* Function Name: SLAVE_I2C_MasterWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SLAVE_I2C_mstrStatus  - The global variable used to store a current
*                                 status of the I2C Master.
*  SLAVE_I2C_state       - The global variable used to store a current
*                                 state of the software FSM.
*  SLAVE_I2C_mstrControl - The global variable used to control the master
*                                 end of a transaction with or without Stop
*                                 generation.
*  SLAVE_I2C_mstrWrBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  SLAVE_I2C_mstrWrBufIndex - The global variable used to store current
*                                    index within the master write buffer.
*  SLAVE_I2C_mstrWrBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No
      ...
      
{
    ...
    errStatus = SLAVE_I2C_MSTR_NOT_READY;

    if(NULL != wrData)
    {
        /* Check I2C state to allow transfer: valid states are IDLE or HALT */
        if(SLAVE_I2C_SM_IDLE == SLAVE_I2C_state)
        {
            /* Master is ready for transaction: check if bus is free */
            if(SLAVE_I2C_CHECK_BUS_FREE(SLAVE_I2C_MCSR_REG))
            {
                errStatus = SLAVE_I2C_MSTR_NO_ERROR;
            }
            else
            {
                errStatus = SLAVE_I2C_MSTR_BUS_BUSY;
            }
        }
        else if(SLAVE_I2C_SM_MSTR_HALT == SLAVE_I2C_state)
        {
            /* Master is ready and waiting for ReStart */
            errStatus = SLAVE_I2C_MSTR_NO_ERROR;

            SLAVE_I2C_ClearPendingInt();
            SLAVE_I2C_mstrStatus &= (uint8) ~SLAVE_I2C_MSTAT_XFER_HALT;
        }
        else
        {
            /* errStatus = SLAVE_I2C_MSTR_NOT_READY was send before */
        }

        if(SLAVE_I2C_MSTR_NO_ERROR == errStatus)
        {
            /* Set state to start write transaction */
            SLAVE_I2C_state = SLAVE_I2C_SM_MSTR_WR_ADDR;

            /* Prepare write buffer */
            SLAVE_I2C_mstrWrBufIndex = 0u;
            SLAVE_I2C_mstrWrBufSize  = cnt;
            SLAVE_I2C_mstrWrBufPtr   = (volatile uint8 *) wrData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            SLAVE_I2C_mstrControl = mode;

            /* Clear write status history */
            SLAVE_I2C_mstrStatus &= (uint8) ~SLAVE_I2C_MSTAT_WR_CMPLT;

            /* Hardware actions: write address and generate Start or ReStart */
            SLAVE_I2C_DATA_REG = (uint8) (slaveAddress << SLAVE_I2C_SLAVE_ADDR_SHIFT);

            if(SLAVE_I2C_CHECK_RESTART(mode))
            {
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterReadBuf
********************************************************************************
*
* Summary:
*  Automatically writes an entire buffer of data to a slave device. Once the
...
*                                 Stop generation.
*  SLAVE_I2C_mstrRdBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  SLAVE_I2C_mstrRdBufIndex - The global variable  used to store a
*                                    current index within the master
*                                    write buffer.
*  SLAVE_I2C_mstrRdBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(NULL != rdData)
    {
        ...
        {
            /* errStatus = SLAVE_I2C_MSTR_NOT_READY was set before */
        }

        if(SLAVE_I2C_MSTR_NO_ERROR == errStatus)
        {
            /* Set state to start write transaction */
            SLAVE_I2C_state = SLAVE_I2C_SM_MSTR_RD_ADDR;

            /* Prepare read buffer */
            SLAVE_I2C_mstrRdBufIndex  = 0u;
            SLAVE_I2C_mstrRdBufSize   = cnt;
            SLAVE_I2C_mstrRdBufPtr    = (volatile uint8 *) rdData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            SLAVE_I2C_mstrControl = mode;

            /* Clear read status history */
            SLAVE_I2C_mstrStatus &= (uint8) ~SLAVE_I2C_MSTAT_RD_CMPLT;
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterSendStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SLAVE_I2C_state - The global variable used to store a current state of
*                           the software FSM.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(SLAVE_I2C_SM_IDLE == SLAVE_I2C_state)
    {
        ...
        if(SLAVE_I2C_CHECK_BUS_FREE(SLAVE_I2C_MCSR_REG))
        {
            ...
            slaveAddress = (uint8) (slaveAddress << SLAVE_I2C_SLAVE_ADDR_SHIFT);
            if(0u != R_nW)
            {
                slaveAddress |= SLAVE_I2C_READ_FLAG;
                SLAVE_I2C_state = SLAVE_I2C_SM_MSTR_RD_ADDR;
            }
            else
            {
            ...
            SLAVE_I2C_DATA_REG = slaveAddress;
            SLAVE_I2C_GENERATE_START_MANUAL;

            /* Wait until address is transferred */
            while(SLAVE_I2C_WAIT_BYTE_COMPLETE(SLAVE_I2C_CSR_REG))
            {
            }
            ...
            if(SLAVE_I2C_CHECK_START_GEN(SLAVE_I2C_MCSR_REG))
            {
                ...
                errStatus = SLAVE_I2C_MSTR_ERR_ABORT_START_GEN;
            }
            else
        #endif /* (SLAVE_I2C_MODE_MULTI_MASTER_SLAVE_ENABLED) */

        #if(SLAVE_I2C_MODE_MULTI_MASTER_ENABLED)
            if(SLAVE_I2C_CHECK_LOST_ARB(SLAVE_I2C_CSR_REG))
            {
                SLAVE_I2C_BUS_RELEASE_MANUAL;

                /* Master lost arbitrage: reset FSM to IDLE */
                SLAVE_I2C_state = SLAVE_I2C_SM_IDLE;
                errStatus = SLAVE_I2C_MSTR_ERR_ARB_LOST;
            }
            else
        #endif /* (SLAVE_I2C_MODE_MULTI_MASTER_ENABLED) */

            if(SLAVE_I2C_CHECK_ADDR_NAK(SLAVE_I2C_CSR_REG))
            {
                /* Address has been NACKed: reset FSM to IDLE */
                SLAVE_I2C_state = SLAVE_I2C_SM_IDLE;
                errStatus = SLAVE_I2C_MSTR_ERR_LB_NAK;
            }
            else
            {
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterSendRestart
********************************************************************************
*
* Summary:
      ...
      
{
    ...
    if(SLAVE_I2C_CHECK_MASTER_MODE(SLAVE_I2C_MCSR_REG))
    {
        /* Set address and read/write flag */
        slaveAddress = (uint8) (slaveAddress << SLAVE_I2C_SLAVE_ADDR_SHIFT);
        if(0u != R_nW)
        ...
        SLAVE_I2C_DATA_REG = slaveAddress;
        SLAVE_I2C_GENERATE_RESTART_MANUAL;
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterSendStop
********************************************************************************
*
* Summary:
...
uint8 SLAVE_I2C_MasterSendStop(void) 
{
    ...
    if(SLAVE_I2C_CHECK_MASTER_MODE(SLAVE_I2C_MCSR_REG))
    {
        SLAVE_I2C_GENERATE_STOP_MANUAL;
        SLAVE_I2C_state = SLAVE_I2C_SM_IDLE;

        /* Wait until stop has been generated */
        while(SLAVE_I2C_WAIT_STOP_COMPLETE(SLAVE_I2C_CSR_REG))
        {
        }
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterWriteByte
********************************************************************************
*
* Summary:
...
uint8 SLAVE_I2C_MasterWriteByte(uint8 theByte) 
{
    ...
    if(SLAVE_I2C_CHECK_MASTER_MODE(SLAVE_I2C_MCSR_REG))
    {
        SLAVE_I2C_DATA_REG = theByte;   /* Write DATA register */
        SLAVE_I2C_TRANSMIT_DATA_MANUAL; /* Set transmit mode   */
        ...
        if(SLAVE_I2C_CHECK_DATA_ACK(SLAVE_I2C_CSR_REG))
        {
            SLAVE_I2C_state = SLAVE_I2C_SM_MSTR_HALT;
            errStatus = SLAVE_I2C_MSTR_NO_ERROR;
        }
        else
        {
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterReadByte
********************************************************************************
*
* Summary:
...
uint8 SLAVE_I2C_MasterReadByte(uint8 acknNak) 
{
    ...
    if(SLAVE_I2C_CHECK_MASTER_MODE(SLAVE_I2C_MCSR_REG))
    {
        ...
        */
        if(SLAVE_I2C_SM_MSTR_RD_ADDR == SLAVE_I2C_state)
        {
            SLAVE_I2C_READY_TO_READ_MANUAL;
        ...
        */
        ...
        {
            SLAVE_I2C_ACK_AND_RECEIVE_MANUAL;
        }
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterStatus
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterClearStatus
********************************************************************************
*
* Summary:
...
uint8 SLAVE_I2C_MasterClearStatus(void) 
{
    ...
    status = SLAVE_I2C_mstrStatus;
    SLAVE_I2C_mstrStatus = SLAVE_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterGetReadBufSize
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SLAVE_I2C_mstrRdBufIndex - The global variable stores current index
*                                    within the master read buffer.
*
*******************************************************************************/
uint8 SLAVE_I2C_MasterGetReadBufSize(void) 
{
    return (SLAVE_I2C_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: SLAVE_I2C_MasterGetWriteBufSize
********************************************************************************
*
* Summary:
*  Returns the amount of bytes that has been transferred with an
...
*
* Global variables:
*  SLAVE_I2C_mstrWrBufIndex -  The global variable used to stores current
*                                     index within master write buffer.
*
*******************************************************************************/
uint8 SLAVE_I2C_MasterGetWriteBufSize(void) 
{
    return (SLAVE_I2C_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: SLAVE_I2C_MasterClearReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SLAVE_I2C_mstrRdBufIndex - The global variable used to stores current
...
/*******************************************************************************
* Function Name: SLAVE_I2C_MasterClearWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SLAVE_I2C_mstrRdBufIndex - The global variable used to stote current
*                                    index within master read buffer.
*  SLAVE_I2C_mstrStatus - The global variable used to store a current
*                                status of the I2C Master.
*
